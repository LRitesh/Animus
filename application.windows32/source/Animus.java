...
public void stateUpdate()
{
  ...
  for(int cIndex = 0; cIndex < columns; cIndex++) {
    for(int rIndex = 0; rIndex < rows; rIndex++) {
      ...
      {
        if(mNeighbors == (int) (sqrt((cIndex-prevX)*(cIndex-prevX) + (rIndex-prevY)*(rIndex-prevY))/(res*1.5f)) && (microbes.size() < maxMicrobes)) 
        {
...
class Microbe {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "Animus" });
  }
}
